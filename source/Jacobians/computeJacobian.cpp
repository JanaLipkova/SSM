...
{
#ifdef Dimerization
    computeJacobianDimerization(Jacobian,X,rates,tau);
#elif defined(LacZLacY)
    computeJacobianLacZLacY(Jacobian,X,rates,tau);
#else
    std::cout<<"This type of jacobian is not supported, see soucrce/Jacobians/ for supported methods";
    abort();
#endif
