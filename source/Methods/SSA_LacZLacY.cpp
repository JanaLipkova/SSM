...
void SSA_LacZLacY::solve()
{
	...
	double genTime = tEnd;   // generation time
	...
	for (int samples = 0; samples < numberOfSamples; ++samples)
	{
		...
		while (t < tEnd)
		{
			...
			// RNAP     = S(1) ~ N(350),35^2)
			// Ribosome = S(9) ~ N(35,3.5^2)
			// + mean values of these pools growth with cell volume so that the concentrations of these molecules remain constant
			simulation->speciesValues(1)  = gennor(350 * (1 + t/genTime), 35);
			simulation->speciesValues(9)  = gennor(35  * (1 + t/genTime),  3.5);
