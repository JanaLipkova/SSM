...
//****************************
void AdaptiveSLeaping::sampling(long int Lcurrent)
{
...
//			}
			...
			if (tau  < (1.0/a0) * sgamma( (double)1.0 ) )
			{
				execute_SSA(type,t,numberOfIterations);
			}
			else 
			{
				Lcurrent = computeLeapLength(tau,type,eventVector); 
			...
			//			else 
			//			{
			//				L = computeLeapLength(tau,type,eventVector); 
			//				sampling(L);
			//				
			//				
			//				if ( isProposedNegative() == false)
